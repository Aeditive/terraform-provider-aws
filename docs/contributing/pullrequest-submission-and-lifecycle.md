...
`gofmt` will also fix many simple formatting issues for you:
...
1. Import declarations are grouped into a maximum of three groups:
    ...
    - Local packages
2. Groups are in that order (i.e., standard packages, third-party packages, and local packages)
3. Groups are separated by a single blank line
4. Packages within each group are alphabetized
